...
export class AppointmentsController {
  ...
  async getCalendarLinks(@Request() req: any, @Param('id') id: string) {
    ...
    return {
      ...
      google: this.appointmentsService.getGoogleCalendarLink(
        appointment,
        tenant,
      ),
      outlook: this.appointmentsService.getOutlookCalendarLink(
        appointment,
        tenant,
      ),
    };
  ...
  async downloadIcal(
    ...
    // Use the tenant from the appointment (already loaded via include)
    const tenant = appointment.tenant;
    const icalContent = this.appointmentsService.generateICalEvent(
      appointment,
      tenant,
    );
