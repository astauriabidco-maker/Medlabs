...
describe('ResultsService', () => {
  ...
  describe('create', () => {
    ...
    it('should upload result and return documentId with accessCode', async () => {
      ...
      prisma.document.create.mockResolvedValue(
        createTestDocument({ id: 'new-doc-id' }),
      );
      prisma.document.findUnique.mockResolvedValue(
        createTestDocument({ id: 'new-doc-id' }),
      );
      prisma.document.update.mockResolvedValue(
        createTestDocument({ status: 'NOTIFIED' }),
      );
    ...
    it('should return paginated results for tenant', async () => {
      ...
      expect(prisma.document.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { tenantId: 'tenant-456', deletedAt: null },
          skip: 0,
          take: 10,
        }),
      );
  ...
  describe('resendResult', () => {
    it('should resend notification and create audit log', async () => {
      ...
      const result = await service.resendResult(
        'tenant-456',
        'doc-123',
        '+237699999999',
        'user-789',
  ...
  describe('remove', () => {
    ...
    it('should throw NotFoundException for non-existent document', async () => {
      ...
      await expect(service.remove('tenant-456', 'nonexistent')).rejects.toThrow(
        NotFoundException,
      );
    });
