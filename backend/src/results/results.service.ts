...
export class ResultsService {
  ...
  async create(
    ...
    if (
      ...
      !file.buffer.slice(0, 4).equals(PDF_MAGIC)
    ) {
    ...
    if (existingDoc) {
      throw new ConflictException(
        `Folder Reference ${createResultDto.folderRef} already exists for this tenant.`,
      );
    }
    ...
    const document = await this.prisma.document.create({
      data: {
        ...
        sampleDate: createResultDto.sampleDate
          ? new Date(createResultDto.sampleDate)
          : null, // Date de prélèvement
      },
    });

    // Trigger Notification (WhatsApp first)
    this.notifyPatient(document.id).catch((err) =>
      console.error('Failed to notify patient', err),
    );

    // Analyze for critical values (async, non-blocking)
    this.analysisService
      .analyzeDocument(document.id, file.buffer)
  ...
  async getPreviewUrl(tenantId: string, resultId: string) {
    ...
    if (doc.status === 'EXPIRED' || doc.deletedAt || doc.purgedAt) {
      throw new ConflictException(
        'Document has been permanently deleted due to retention policy.',
      );
    }
  ...
  async resendResult(
    ...
    if (doc.status === 'EXPIRED' || doc.deletedAt || doc.purgedAt) {
      throw new ConflictException(
        'Cannot resend result: Document has been deleted due to retention policy.',
      );
    }
  ...
  async resendAccessCode(tenantId: string, resultId: string) {
    ...
    if (!doc || doc.deletedAt || doc.purgedAt || doc.status === 'EXPIRED')
      throw new NotFoundException('Document not found');
    if (!doc.accessCode)
      throw new BadRequestException(
        'No access code available for this document',
      );
    ...
    return {
      ...
      maskedPhone:
        doc.patientPhone.slice(0, 7) + '****' + doc.patientPhone.slice(-2),
    };
  }
  ...
  async remove(tenantId: string, id: string, userId?: string) {
    ...
    await this.prisma.auditLog.create({
      data: {
        ...
        description: `Document queued for purge: ${doc.folderRef}`,
        resourceId: id,
        actorId: userId,
      },
    });
    ...
    return {
      message: 'Document deleted successfully. Physical purge has been queued.',
    };
  }
  ...
  async compareResults(tenantId: string, resultIds: string[]) {
    if (!resultIds || resultIds.length < 2) {
      throw new BadRequestException(
        'At least 2 results are required for comparison',
      );
    }
    if (resultIds.length > 10) {
      throw new BadRequestException(
        'Maximum 10 results can be compared at once',
      );
    }
