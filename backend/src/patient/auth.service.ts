...
export class PatientAuthService {
  ...
  async requestOtp(
    ...
    try {
      ...
      if (!result.success) {
        ...
        if (!smsResult.success) {
          this.logger.warn(
            `Failed to send OTP to ${normalizedPhone}: ${smsResult.error}`,
          );
    ...
    this.logger.log(
      `OTP requested for ${normalizedPhone.slice(-4)} on tenant ${tenantId}`,
    );
  ...
  async verifyOtp(
    ...
    if (!session) {
      throw new UnauthorizedException(
        'Session expirée. Veuillez redemander un code.',
      );
    }
    ...
    if (new Date() > session.expiresAt) {
      ...
      throw new UnauthorizedException(
        'Code expiré. Veuillez redemander un code.',
      );
    }
    ...
    this.logger.log(
      `Patient authenticated: ${normalizedPhone.slice(-4)} on tenant ${tenantId}`,
    );
