...
export class PatientAuthController {
  ...
  private createPaymentAccessToken(documentId: string): string {
    return this.jwtService.sign(
      { sub: documentId, type: 'patient_payment' },
      { secret: this.getPatientSecret(), expiresIn: '10m' },
    );
  }

  private assertDocumentAccessible(document: any): void {
    if (
      ...
      throw new HttpException(
        'Document has expired or has been deleted',
        HttpStatus.GONE,
      );
    }
  ...
  async verifyFallback(@Body() body: { token: string; dob: string }) {
    ...
    if (!token || !dob)
      throw new BadRequestException('Token and Date of Birth are required');
    ...
    if (!otpRecord) {
      ...
      throw new BadRequestException(
        'Session invalid or expired. Please restart the process.',
      );
    }
    ...
    if (!document.patientDob) {
      throw new BadRequestException(
        'Fallback authentication unavailable for this document. Please contact the Lab.',
      );
    }
   ...
   */
  @Post('verify-code')
  @Throttle({ default: { limit: 8, ttl: 60_000, blockDuration: 15 * 60_000 } })
  @HttpCode(HttpStatus.OK)
  async verifyCode(@Body() body: { token: string; accessCode: string }) {
    const { token, accessCode } = body;
    if (!token || !accessCode)
      throw new BadRequestException('Token and access code are required');
    ...
    if (!document.accessCode) {
      throw new BadRequestException(
        'No access code available for this document. Please contact the laboratory.',
      );
    }
    ...
    if (normalizedInput !== normalizedStored) {
      throw new ForbiddenException(
        'Invalid access code. Please check the code on your receipt.',
      );
    }
    ...
    if (paymentStatus === 'UNPAID' && price > 0) {
      ...
      return {
        ...
        paymentAccessToken: this.createPaymentAccessToken(document.id),
      };
    }
